...
function RadarChart({ labels, values, size = 340 }: { labels: string[]; values: number[]; size?: number }) {
  ...
  const dataCoords = values.map((v, i) => pointFor(v, i));
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} aria-label="Gráfico radar 0–100%">
      ...
      <polygon points={dataPoints} fill="rgba(37,99,235,0.30)" stroke="#2563eb" strokeWidth={2} />
      {labels.map((label, i) => {
...
export default function ViewMaturidadePage({ params }: { params: { id: string } }) {
  ...
  async function exportPDF() {
    ...
    const labelsArr = summaries.map((s) => s.name);
    const percentsArr = summaries.map((s) => s.percent);
    const sizeSvg = 360;
    const centerSvg = sizeSvg / 2;
    const radiusSvg = centerSvg - 24;
    const angleStepSvg = (2 * Math.PI) / labelsArr.length;
    const pointForSvg = (val: number, i: number) => {
      const angle = -Math.PI / 2 + i * angleStepSvg;
      const r = (Math.max(0, Math.min(100, val)) / 100) * radiusSvg;
      const x = centerSvg + r * Math.cos(angle);
      const y = centerSvg + r * Math.sin(angle);
      return [x, y] as const;
    };
    const levelVals = [25, 50, 75, 100];
    const polygonPointsSvg = (lvl: number) => Array.from({ length: labelsArr.length }, (_, i) => pointForSvg(lvl, i)).map(([x, y]) => `${x},${y}`).join(' ');
    const dataCoordsSvg = percentsArr.map((v, i) => pointForSvg(v, i));
    const dataPointsSvg = dataCoordsSvg.map(([x, y]) => `${x},${y}`).join(' ');
    const svg = `
      <svg width="${sizeSvg}" height="${sizeSvg}" viewBox="0 0 ${sizeSvg} ${sizeSvg}" aria-label="Gráfico radar 0–100%">
        ${levelVals.map((lvl) => `<polygon points="${polygonPointsSvg(lvl)}" fill="none" stroke="#e5e7eb" stroke-width="1" />`).join('')}
        ${labelsArr.map((_, i) => {
          const [x, y] = pointForSvg(100, i);
          return `<line x1="${centerSvg}" y1="${centerSvg}" x2="${x}" y2="${y}" stroke="#e5e7eb" stroke-width="1" />`;
        }).join('')}
        <polygon points="${dataPointsSvg}" fill="rgba(37,99,235,0.30)" stroke="#2563eb" stroke-width="2" />
        ${labelsArr.map((label, i) => {
          const [x, y] = pointForSvg(85, i);
          const dy = y > centerSvg ? 12 : -6;
          return `<text x="${x}" y="${y}" text-anchor="middle" font-size="11" fill="#374151" dy="${dy}">${label}</text>`;
        }).join('')}
        ${levelVals.map((lvl) => `<text x="${centerSvg}" y="${centerSvg - (lvl / 100) * radiusSvg}" text-anchor="middle" font-size="10" fill="#9ca3af">${lvl}%</text>`).join('')}
      </svg>
    `;

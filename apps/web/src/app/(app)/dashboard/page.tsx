    ...
    Promise.all([
      ...
      apiGet<MaturityListResponse>("/maturity", token),
        ...
        try {
          ...
          companies.forEach((c: any) => companyNameById.set(c.id, (c.fantasyName || c.name)));
          let totalSubnets = 0;
          let totalUsed = 0;
          let totalCapacity = 0;
          ...
          for (const comp of companies) {
            ...
            const sitesRes = await apiGet<any[]>(`/sites?companyId=${comp.id}`, token);
            const sitesList = Array.isArray(sitesRes) ? sitesRes : [];
            const siteNameById = new Map<string, string>();
            sitesList.forEach((s: any) => siteNameById.set(s.id, s.name));
            for (const s of statsList) {
              ...
              const companyName = companyNameById.get(comp.id) || (comp.fantasyName || comp.name);
              const siteName = s.siteId ? (siteNameById.get(String(s.siteId)) || '') : '';
              const key = `${comp.id}|${String(s.siteId || '')}|${String(s.name).trim().toUpperCase()}|${s.cidr}`;
              ...
              <div className="space-y-1">
                {topSubnets.map((t, i) => (
                  <div key={i} className="grid grid-cols-12 items-center gap-2 text-xs">
                    <div className="col-span-2 truncate">{t.company || 'Empresa'}</div>
                    <div className="col-span-2 truncate">{t.site || 'Site'}</div>
                    <div className="col-span-5 truncate">{t.name} <span className="text-muted">({t.cidr})</span></div>
                    <div className="col-span-2 h-2 bg-border rounded">
                      <div className={`h-2 rounded ${t.occ >= 90 ? 'bg-red-600' : t.occ >= 70 ? 'bg-yellow-500' : 'bg-green-600'}`} style={{ width: `${t.occ}%` }} />
                    </div>
                    <div className="col-span-1 text-right">{t.occ}%</div>
                  </div>
                ))}
              </div>
            )}

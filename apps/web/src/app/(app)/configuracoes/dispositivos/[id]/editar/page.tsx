...
export default function EditDevicePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await apiGet<{ ok: boolean; data?: any }>(`/devices/${deviceId}`, token);
        const d = res?.data;
        if (d) {
          setDeviceTypeId(d.deviceTypeId || "");
          setBrandId(d.brandId || "");
          setModel(d.model || "");
          setUHeight(d.uHeight != null ? String(d.uHeight) : "");
          setIsFullDepth(Boolean(d.isFullDepth));
          setFrontImage(Boolean(d.frontImage));
          setRearImage(Boolean(d.rearImage));
          setWeight(d.weight != null ? String(d.weight) : "");
          setWeightUnit(d.weightUnit || "kg");
          setAirflow(d.airflow || "");
          setConsolePorts(Array.isArray(d.consolePorts) ? d.consolePorts : []);
          setInterfaces(Array.isArray(d.interfaces) ? d.interfaces : []);
          setModuleBays(Array.isArray(d.moduleBays) ? d.moduleBays : []);
        }
      } catch {}
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      const res = await apiPut<{ ok: boolean; data?: any; error?: string }>(`/devices/${deviceId}`, token, payload);
      if (!res?.ok) {
        setError(res?.error || "Erro ao salvar dispositivo.");
      } else {
        setMsg("Dispositivo atualizado com sucesso.");
        router.push(`/configuracoes/dispositivos`);
      }
    } catch (e: any) {
      setError("Falha ao atualizar dispositivo.");
    } finally {
      setSaving(false);
    }
  }
      ...
      {loadingPerms ? (
        ...
        <div className="bg-card border border-border rounded p-4">
          ...
          <div className="mt-4 flex items-center gap-2">
            <button disabled={saving} className="px-3 py-2 rounded bg-primary text-white hover:bg-primary/90 disabled:opacity-50" onClick={handleSave}>Salvar alterações</button>
